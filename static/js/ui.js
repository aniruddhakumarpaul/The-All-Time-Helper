...
function updUI() {
    if (state.user) {
        ...
        if (avCont) avCont.innerHTML = `<div class="av u-av" style="width: 32px; height: 32px; font-size: 0.8rem;"><span class="initial-letter">${initial}</span><span class="full-name">${nameStr}</span></div>`;
    }
}
...
function addMsg(r, c, i, idx, mName, isMasked = false) {
    ...
    div.innerHTML = `
        ...
        <div class="txt" draggable="false" ondragstart="if(!window.isGDown) { event.preventDefault(); return false; } handleDragStart(event, this.innerText)" ondragend="handleDragEnd(event)">
            ...
            ${i ? `<div class="chat-img-preview-container" onclick="openImageModal('data:image/png;base64,${i}')"><img src="data:image/png;base64,${i}" class="chat-img-preview"></div>` : ''}
            ${watermark}
            ${tools}
        </div>
    `;
