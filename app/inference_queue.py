...
class InferenceQueue:
    ...
    async def submit(self, job_id: str, fn: Callable, abort_event: threading.Event, timeout: float = DEFAULT_INFERENCE_TIMEOUT, owner: str = "") -> Any:
        """
    ...
    def cancel(self, job_id: str, owner: str = "") -> bool:
        ...
        if owner and job.owner and job.owner != owner:
            logger.warning(f"[InferenceQueue] Rejecting cancel for job {job_id}: owner mismatch")
            return False

        job.abort_event.set()
        if not job.result_future.done():
            job.result_future.set_result("Operation cancelled.")
        logger.info(f"[InferenceQueue] Cancelled job {job_id}")
        return True
