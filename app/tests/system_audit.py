...
def test_hybrid_email_mode():
    ...
    try:
        ...
        if "EMAIL_DRAFT_PAYLOAD" in str(e):
            print("SUCCESS: Native Tool correctly intercepts emails (AgentFastExit).")
            return True
        else:
            print(f"FAIL: Email tool raised AgentFastExit with unexpected payload: {str(e)}")
            return False
    except Exception as e:
