...
def _find_pending_sensitive_request(history: list[dict]) -> str:
    """Recover the last real user request before admin-key challenge turns."""
...
async def chat_endpoint(req: ChatRequest, request: Request, current_user: str = Depends(get_current_user)):
    ...
    if req.isMasked:
        ...
        if pending_request:
            prompt = (
                "ADMIN_KEY_PROVIDED: The system has securely received and validated the Admin Key. "
                "Execute this pending sensitive request without exposing the key:\n\n"
                f"{pending_request}"
            )
        else:
            prompt = (
                "ADMIN_KEY_PROVIDED: The system has securely received and validated the Admin Key, "
                "but no pending sensitive request was found in chat history. Ask the user to repeat the action request."
            )
    else:
        admin_auth_context.set(None)
