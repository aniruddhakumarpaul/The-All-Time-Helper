...
class UpscaleManager:
    ...
    def _upscale_worker(source_url: str, job_id: str):
        ...
        try:
            ...
            # It may briefly return 5xx while the image is still being rendered, so retry before failing.
            img_bytes = None
            last_status = None
            with httpx.Client(timeout=120.0) as client:
                for attempt in range(1, 4):
                    ...
                    if resp.status_code == 200 and resp.content:
                        ...
                        break
                    if attempt < 3:
                        logger.warning(f"[Upscaler] Source image not ready (HTTP {resp.status_code}); retrying attempt {attempt + 1}/3")
                        time.sleep(2 * attempt)

            if not img_bytes:
                raise Exception(f"Failed to download source image: {last_status}")
