...
def _extract_username(text: str) -> str | None:
    raw = str(text or "")
    explicit = re.search(r"(?:username|handle)\s*[:=]?\s*@?([a-zA-Z0-9._]{1,30})\b", raw, flags=re.I)
    if explicit:
    ...
    stopwords = {"search", "web", "for", "profile", "link", "username", "handle", "instagram", "insta", "ig", "github", "twitter", "x", "tiktok", "find", "lookup", "url", "the", "a", "an", "of"}
