...
from app.logic.memory import delete_memory, index_document, is_memory_available, query_memory, user_context
...
# Configuration
IGNORE_DIRS = {'.project_brain', '.project_brain_active', '.project_brain_backup', '__pycache__', 'node_modules', '.git', 'venv', 'practice', '_archived_modules', 'scratch'}
...
def rebuild_neural_memory():
    ...
    if not is_memory_available():
        raise RuntimeError("ChromaDB is unavailable. Stop the app server or repair .project_brain before rebuilding memory.")

    print(f"DEBUG: Clearing existing neural memory for user: {TARGET_USER}")
    delete_memory("", user_id=TARGET_USER, clear=True)
        ...
        for file in files:
            ...
            try:
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    discovered += 1
                    if index_document(doc_id, chunk["content"], chunk["metadata"]):
                        count += 1
    ...
    for rule in MANUAL_TOOL_RULES:
        ...
        discovered += 1
        if index_document(doc_id, rule["content"], {"type": "tool_rule", "tool": rule["tool"]}):
            count += 1

    if count != discovered:
        raise RuntimeError(f"Memory rebuild incomplete: wrote {count}/{discovered} discovered chunks. Errors: {errors}")

    smoke = query_memory("cloud model routing gemma openrouter agents", n_results=3, user_id=TARGET_USER)
    print(f"\nSUCCESS: Indexed {count} semantic chunks (AST-aware + {len(MANUAL_TOOL_RULES)} rules). Errors: {errors}")
    print(f"DEBUG: Smoke query returned {len(smoke)} result(s).")
