...
- Newly created jobs also carry a short `claim_expires_at` provisional launch deadline. This closes the create-before-claim crash window without changing the public active-job protocol; a valid worker atomically claims before the deadline, while an unclaimed expiry becomes `The server stopped before this response could start. Please retry.` and is never replayed.
...
- Admission reserves one bounded terminal result per active job. During writes, expired/old terminal rows and optional non-final events are compacted before projected logical usage is checked; progress retention may be rejected without stopping inference, but terminal content is fitted or rejected before commit and is never discarded because stream history consumed the budget.
