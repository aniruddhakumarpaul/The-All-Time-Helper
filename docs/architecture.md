...
- `GET /chat/jobs/{job_id}` reconnects an authenticated owner to an active or completed response for up to one hour; the registry is intentionally process-local, so a server shutdown ends the job rather than pretending it completed.
- `app/logic/chat_job_registry.py`: bounded, owner-scoped in-memory job events and terminal results; a disconnected browser stream cannot cancel execution, while Stop and follow-up prompts still signal the queue abort event.
