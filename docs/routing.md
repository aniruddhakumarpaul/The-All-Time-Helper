...
- Deterministic heuristics run before LLM classification for exact-output replies and explicit email, image, and web-search actions; ambiguous requests retain structured classification.
...
- Proven model-free image generation, visual continuation, and explicit web/image search requests use a bounded tool lane, so they do not wait behind serialized GPU inference. Email, attached-image, sensitive, persona, capability-discussion, and ambiguous requests stay on the inference lane.
